...
df[outlier_cols[0:3] + ["label"]].boxplot(by="label", figsize=(20,10), layout=(1,3))

#plotting Gyrocsope data
...
def mark_outliers_iqr(dataset, col):
    ...
    Args:
        dataset (pd.DataFrame): the dataset
        col (str): column on which to apply outlier detection
        
    Returns:
        pd.DataFrame: the orignal dataframe with an
        extra columns marking whether the value is an outlier or not
    """
...
# ! Chauvenets criterion assumes a normal distribution
# check for normal distribution
df[outlier_cols[0:3] + ["label"]].plot.hist(by="label", figsize=(20,20), layout=(3,3))
...
#print total outliers dropped        
n = len(outliers_removed_df) - len(outliers_removed_df.dropna(axis=0))
